 ...
 *
 * @brief Adds a weight whenever a operator or a class is required to be in two 
 * rooms at the same time.
 *
 * Please note that this module will not consider it as an error if a operator or
 ...
 *
 * This restriction specifies that current class or operator should never have 
 * lessons at the same time as the class or operator specified in the 
 * restriction.
 *
 * There are two common uses for this restriction: When multiple classes share
 * an event and when multiple operators share an event. Consider the following 
 * example:
 * 
 ...
 *
 * Multiple operators per event can be defined in a similar way.
 */
...
int module_fitness(chromo **c, ext **e, slist **s)
{
	...
	operator_type=operator->restype;
	...
	sum=0;
        for(m=0;m<time->gennum;m++) {
			...
			b=list->tupleid[a][n];
				...
				if(res_get_conflict(operator_type, 
							operator->gen[m], 
							operator->gen[b])) {
					sum++;
				}
...
int module_precalc(moduleoption *opt)
{
	...
	operator=restype_find("operator");

	if(recursive) debug("Recursive conflicts were enabled");

	if(!operator->var) {
		if(check_time(operator)) return -1;
	}
...
int module_init(moduleoption *opt) 
{
	...
	handler_res_new("operator", "conflicts-with", getconflict);
